const modalElementInnerHTML = `
    <div class="modal-content">
        ...
        <p id="swquiz-game-summary" class="sub-title">The force is strong in you young Padawan! During 2 minutes you have answered X / Y
            questions. And Computer guessed X / Y.</p>
...
export const GameResultModal = () => {
  ...
  const modal = {
    ...
    show({data}) {
      const computerAnswersStats = answersStats({forPlayer: 'computer', answers: data.answers})
      ...
      summaryElement.innerText = `The force is strong in you young Padawan! During 2 minutes you have answered ${humanAnswersStats.correct} / ${humanAnswersStats.all} questions. And Computer guessed ${computerAnswersStats.correct} /  ${computerAnswersStats.all}.`
...
const AnswersRowElement = ({answer}) => {
  ...
  const {image, correctAnswerName, humanAnswer, computerAnswer} = answer;
  ...
  computerAnswerNameElement.innerText = computerAnswer ? computerAnswer.answerName : "None"
  computerAnswerNameElement.className = computerAnswer && computerAnswer.isCorrect ? "sw-quiz-answers-table-correct" : "sw-quiz-answers-table-incorrect"
  ...
  rowContentElement.appendChild(computerAnswerNameElement)
